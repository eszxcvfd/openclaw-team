...
import { N as isRootHelpInvocation, P as isRootVersionInvocation, j as hasHelpOrVersion } from "./logger-BG-0yWA-.js";
...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-PlEi6ESk.js");
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-Bno2hXTJ.js"), import("./git-commit-DJlLc3sp.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-CcBQB6qx.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-CJttwYwz.js").then(({ runCli }) => runCli(argv)).catch((error) => {
