...
import { A as hasFlag, D as getPositiveIntFlagValue, E as getFlagValue, L as isValueToken, N as isRootHelpInvocation, O as getPrimaryCommand, T as getCommandPositionalsWithRootOptions, j as hasHelpOrVersion, k as getVerboseFlag, w as getCommandPathWithRootOptions } from "./logger-BG-0yWA-.js";
...
import { a as enableConsoleCapture, m as defaultRuntime } from "./subsystem-YgTuVQVh.js";
import "./ansi-cwY8Vrne.js";
import "./boolean-B6zcAynR.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-CQLG1scp.js";
import { n as VERSION } from "./version-GCXYdDuZ.js";
import "./logging-Bk1RvIn5.js";
import { t as assertSupportedRuntime } from "./runtime-guard-BiaA8gCk.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-Ia_x1HSo.js");
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		if (timeoutMs === null) return false;
		if (json) {
			const { statusJsonCommand } = await import("./status-json-BxHSzbd6.js");
		...
		const { statusCommand } = await import("./status-CTqovNVi.js");
		await statusCommand({
			json,
			deep,
			all,
...
const routeGatewayStatus = {
	...
	run: async (argv) => {
		...
		const { runDaemonStatus } = await import("./status-7FeTPbef.js");
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DjqPyxgn.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-TCdlvU6x.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-Dc6pHHMG.js");
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-DwgO6qTN.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-DwgO6qTN.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-CCBbbyPH.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-CCBbbyPH.js");
...
async function prepareRoutedCommand(params) {
	...
	const { ensureConfigReady } = await import("./config-guard-CpPJvCpc.js");
	...
	if (typeof params.loadPlugins === "function" ? params.loadPlugins(params.argv) : params.loadPlugins) {
		const { ensurePluginRegistryLoaded } = await import("./plugin-registry-CCsPPB0z.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-cWCrwZ7x.js");
...
async function runCli(argv = process$1.argv) {
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputRootHelp } = await import("./root-help-CcBQB6qx.js");
			outputRootHelp();
			return;
		}
		if (await tryRouteCli(normalizedArgv)) return;
		enableConsoleCapture();
		const { buildProgram } = await import("./program-Fi_1RoT4.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-Kbxv8kjE.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-B5SAOMlT.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-Bz4l3TR-.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-Bz4l3TR-.js");
