...
describe('AuditController integration', () => {
  ...
  it('coerces list query params and returns successful rows with pagination metadata', async () => {
    ...
    const response = await request(app.getHttpServer())
      .get(
        '/api/audit-logs?page=2&pageSize=10&user=alice&tool=generate&success=true&dateFrom=2026-03-21&dateTo=2026-03-22&trace=trace-success',
      )
  ...
  it('returns standardized forbidden error for non-admin access', async () => {
    ...
    deniedApp.useGlobalInterceptors(new SuccessResponseInterceptor());
    await deniedApp.init();

    const response = await request(deniedApp.getHttpServer())
      .get('/api/audit-logs')
